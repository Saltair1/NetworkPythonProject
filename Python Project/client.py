...
localDNSIP = 'DNS IP address'  # replace later
localDNSPort = 15000
...
while True:
    ...
    dns_query = transaction_id.to_bytes(4, byteorder='big') + (qr << 28 | type_flags << 24 | name_length).to_bytes(4, byteorder='big') + value_length.to_bytes(4, byteorder='big') + query_name.encode()
